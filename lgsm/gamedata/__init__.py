...
class GameData(object):
	...
	def load_file(self, file):
		...
		try:
			dict_merge(target=data, source=conf)
		except:
			pass
			...
			for impfile in data["import"]:
				try:
					...
					if len(impdata.keys()) > 0:
						...
						dict_merge(target=impdata, source=data)
						dict_merge(target=data, source=impdata)
				except:
					data['failures'] = "import %s" % impfile
		return data
